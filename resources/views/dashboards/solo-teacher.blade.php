...
@section('page_title', 'Solo Teacher Dashboard')
...
<div class="space-y-6">
    <div class="bg-gradient-to-r from-maroon-800 via-maroon-700 to-maroon-600 rounded-2xl p-6 text-white animate-slide-down relative overflow-hidden">
        ...
        <div class="relative z-10">
            ...
            <p class="text-maroon-100 text-sm">Your creator dashboard — revenue, students, courses, and wallet.</p>
            <div class="mt-4 flex gap-3">
                ...
                <a href="{{ route('marketplace.index') }}" class="px-4 py-2 bg-white/10 hover:bg-white/20 rounded-xl text-sm font-bold transition-all">Browse Marketplace</a>
            </div>
        </div>
    </div>
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 stagger">
        @php
            $kpis = [
                ['label' => 'Total Revenue', 'value' => '$' . number_format($totalRevenue, 2), 'icon' => 'M12 8c-1.657 0-3 .895-3 2s1.343 2 3 2 3 .895 3 2-1.343 2-3 2', 'gradient' => 'from-success-400 to-success-600', 'subtitle' => 'Course sales'],
                ['label' => 'Wallet Balance', 'value' => '$' . number_format($walletBalance, 2), 'icon' => 'M17 9V7a2 2 0 00-2-2H5a2 2 0 00-2 2v6a2 2 0 002 2h2', 'gradient' => 'from-maroon-500 to-maroon-700', 'subtitle' => 'Available for payout'],
                ['label' => 'Total Students', 'value' => $totalStudents, 'icon' => 'M17 20h5v-2a4 4 0 00-3-3.87', 'gradient' => 'from-info-400 to-info-600', 'subtitle' => 'Enrolled across courses'],
                ['label' => 'Completion Rate', 'value' => $completionRate . '%', 'icon' => 'M9 12l2 2 4-4', 'gradient' => 'from-orange-400 to-orange-600', 'subtitle' => $totalCourses . ' courses'],
            ];
        @endphp
        @foreach($kpis as $kpi)
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div class="lg:col-span-2 bg-white rounded-2xl p-6 shadow-sm border border-gray-100 animate-slide-up">
            <h3 class="font-bold text-gray-800 mb-4">Sales & Enrollment (30 days)</h3>
        ...
        <div class="bg-white rounded-2xl p-6 shadow-sm border border-gray-100 animate-slide-up">
            <h3 class="font-bold text-gray-800 mb-4">Top Courses</h3>
            <div class="space-y-4">
    ...
    <div class="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden animate-slide-up">
        <div class="p-5 border-b border-gray-100 flex items-center justify-between">
            ...
            <a href="{{ route('wallet.index') }}" class="text-xs font-bold text-maroon-500 hover:text-maroon-700">View Wallet →</a>
        </div>
        <div class="divide-y divide-gray-50">
            ...
            @empty
                <div class="p-8 text-center text-gray-400 text-sm">No students yet</div>
            @endforelse
        </div>
    </div>
</div>
